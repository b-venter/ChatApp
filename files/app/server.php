...
$host = $websocket_host1; //host
$port = $websocket_port1; //port
$null = NULL; //null var
...
while (true) {
	...
	foreach ($changed as $changed_socket) {	
		...
		while(socket_recv($changed_socket, $buf, 1024, 0) >= 1)
		{
			...
			$db_save = $link -> prepare("INSERT INTO `messages`(`time`, `sender`, `message`) VALUES (NOW(),?,?)");
			$db_save->bind_param("ss", $user_name, $user_message);
			$db_save->execute();
			$db_save->close();
